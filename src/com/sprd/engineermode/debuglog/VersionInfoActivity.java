...
public class VersionInfoActivity extends Activity {
    ...
    public static final String GPS_CHIP_TYPE = SystemPropertiesProxy.get("ro.wcn.gpschip", DEFAULT_GPS_CHIP_TYPE);
    ...
    private String recombinResult (int cmdType, String res){
        ...
        if (cmdType == GET_CP2_VERSION) {
            /* SPRD Bug 826395:Parse cp2 version information in marlin3 phone. @{ */
            if (res.startsWith("marlin")) {
                ...
                if (res.startsWith("Platform")) {
                    ...
                    m = Pattern.compile(PROC_VERSION_REGEX).matcher(res);
                    if (!m.matches()) {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case GET_CP2_VERSION:
                    String powerResult = SocketUtils.sendCmdAndRecResult(SOCKET_NAME,
                            LocalSocketAddress.Namespace.ABSTRACT, PROCESS_NAME + "wcn poweron");
                    Log.d(TAG,"OPEN_CP2 Response is " + powerResult);
                    result = SocketUtils.sendCmdAndRecResult(SOCKET_NAME,
