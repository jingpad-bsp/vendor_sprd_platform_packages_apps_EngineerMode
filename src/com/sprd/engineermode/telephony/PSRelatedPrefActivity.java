...
import com.sprd.engineermode.utils.SimSelectHelper;
...
public class PSRelatedPrefActivity extends PreferenceActivity implements
    ...
    private static final String C2K_FREQUENCY = "c2k_frequency";
    ...
    private boolean isCardExist() {
        int phoneCount = TelephonyManagerProxy.getPhoneCount();
        int cardExistNum = 0;
        TelephonyManager telephonyManager = TelephonyManagerProxy.getService();
        for (int i = 0; i < phoneCount; i++) {
            if (telephonyManager != null
                    && telephonyManager.getSimState(i) == TelephonyManager.SIM_STATE_READY) {
                cardExistNum++;
            }
        }
        if (cardExistNum >= 1) {
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen,
        ...
        if (key.equals(SIMINFO)) {
            ...
            Intent intent = new Intent("android.engineermode.action.GSMFREQUENCY");
            startActivity(intent);
        } else if (key.equals(OTHERS_FREQUENCY)) {
            Intent intent = new Intent("android.engineermode.action.LOCKFREQUENCY");
            startActivity(intent);
        } else if (key.equals(C2K_FREQUENCY)) {
            Intent intent = new Intent("android.engineermode.action.LOCKC2KFREQUENCY");
            startActivity(intent);
        } else {
            return false;
        }
