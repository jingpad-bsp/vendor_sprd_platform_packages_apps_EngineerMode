        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case FPLMN: {
                    String cmd = null;
                    mInfo = "";
                    for (int simIndex = 0; simIndex < TelephonyManagerProxy.getPhoneCount(); simIndex++) {
                        String plmnInfo;
                        try {
                            plmnInfo = teleApi.simFunc().readPlmn(simIndex);
                        } catch (Exception e) {
                            e.printStackTrace();
                            break;
                        }

                        mInfo += String.format("SIM%d:\n%s", simIndex+1, plmnInfo);
                    }
                ...
                case NETINFOSTATIS: {
                    String plmn = "";
                    try {
                        plmn = teleApi.netInfo().getPlmn();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    final String plmnInfo = plmn;
                    mUiThread.post(new Runnable() {

                        @Override
                        public void run() {
                            mTextView.setText(plmnInfo);
                        }
                ...
                case SIM_INFO: {
                    mInfo = "";
                    for (int i = 0, simIndex = 1; i < TelephonyManagerProxy.getPhoneCount(); i++, simIndex++) {
