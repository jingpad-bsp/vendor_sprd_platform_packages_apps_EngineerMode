...
public class GLockFrequencyActivity extends Activity {
    ...
    class GsmLockFreqHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
            case GET_GSM_LOCK_FREQ:
                atCmd = engconstents.ENG_AT_SPGSMFRQ1;
            ...
            case SET_GSM_LOCK_FREQ:
                atCmd = engconstents.ENG_AT_SPGSMFRQ + "1";
                for (int i = 0; i < DEFAULT_LOCK_MAX; i++) {
                    if (!mEditFreq[i].getText().equals("")) {
                        atCmd += "," + mEditFreq[i].getText().toString().trim();
                    }
                }
                responValue = IATUtils.sendATCmd(atCmd, mServerName);
                Log.d(TAG, "resluts of " + atCmd + "is :" + responValue);
            ...
            case SET_GSM_UNLOCK_FREQ:
                atCmd = engconstents.ENG_AT_SPGSMFRQ + "0";
                responValue = IATUtils.sendATCmd(atCmd, mServerName);
                Log.d(TAG, "resluts of " + atCmd + "is :" + responValue);
                if (responValue != null && responValue.contains(IATUtils.AT_OK)) {
                    mUiThread.post(new Runnable() {
