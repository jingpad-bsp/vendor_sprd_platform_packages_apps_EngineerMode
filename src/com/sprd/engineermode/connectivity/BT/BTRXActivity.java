...
public class BTRXActivity extends Activity implements OnClickListener,OnItemSelectedListener {
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position,
        ...
        if (mBTRx != null) {
            if (parent == mPacType) {
                ...
                mBTRx.pattern = this.getResources().getStringArray(R.array.bt_rx_pattern_int)[position];
            }
        }
    }
    ...
    private void initUI(){
        mPattern = (Spinner) findViewById(R.id.bt_rx_pattern);
        ArrayAdapter<String> patternAdapter = new ArrayAdapter<String>(this,
                android.R.layout.simple_spinner_item, getResources()
                        .getStringArray(R.array.bt_rx_pattern));
        patternAdapter
        ...
        public void handleMessage(Message msg) {
            switch(msg.what) {
                case MSG_BT_RX_START:
                    if (btEut.BTRxStart(mBTRx)) {
                        mUiThread.post(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(BTRXActivity.this, "RX Start Success",
                                        Toast.LENGTH_SHORT).show();
                ...
                case MSG_BT_RX_READ:
                    ...
                    if (mRxResult != null && mRxResult.contains("OK")) {
                        ...
                        if (mResultAdapter.result.size() % 2  == 0) {
                            mSingleRes = new BTRXResult(mCurRssi, mCurPer, mCurBer, true);
                        } else {
                            mSingleRes = new BTRXResult(mCurRssi, mCurPer, mCurBer, false);
                        }
                  ...
                  /*      mUiThread.post(new Runnable() {
                    ...
                    break;
                case MSG_BT_RX_STOP:
                    if (btEut.BTRxStop(mBTRx)) {
                        mUiThread.post(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(BTRXActivity.this, "RX Stop Success",
                                        Toast.LENGTH_SHORT).show();
